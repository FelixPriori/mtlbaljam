    ...
    <section className={styles.bandSection}>
      <div className={styles.bandName}>
        <div className={styles.logoWrapper}>
          <Image src="/legacy-band.png" alt={musicSection.bandName} width={270} height={291} />
        </div>
        <div className={styles.text}>
          <h2>{musicSection.title}</h2>
          <h3>{musicSection.bandName}</h3>
          <p>{musicSection.description}</p>
          <Link href={`/${lang}${musicSection.learnMore.href}`}>{musicSection.learnMore.text}</Link>
        </div>
      </div>

      <div className={styles.socialsWrapper}>
        <IconBox src="/mbj-toaster-black.png" alt={iconAlts.toaster} width={50} height={50} position="topRight" />
